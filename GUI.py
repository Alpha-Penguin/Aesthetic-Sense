...
from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QLabel, \
    ...
    QTextEdit, QFileSystemModel, QTreeView, QTreeWidget, QAction, QGraphicsView, QSizePolicy
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.analyse_btn.clicked.connect(self.load_images)  # Connect button to function
        ...
        self.analyse_current_btn.clicked.connect(self.load_images)
        self.analyse_current_btn.setVisible(False)  # Hidden initially

        self.analyse_selected_btn = QPushButton("Analyse Selected")
        self.analyse_selected_btn.clicked.connect(self.load_images)
        ...
        self.full_image_label.setFixedSize(1000, 800)  # Set fixed size for full image
        self.full_image_label.setVisible(False)  # Initially hidden

        # Metadata Panel
        self.metadata_panel = QTextEdit()
        self.metadata_panel.setReadOnly(True)
        self.metadata_panel.setFixedWidth(250)  # Adjust width as needed
    ...
    def on_item_clicked(self, index):
        ...
        print(self.image_paths.index(file_path))
        ...
        # Now get the child widget of the QVBoxLayout
        # vbox is your QVBoxLayout, and it contains the label (image) and stars_label
        ...
        line += "\n ⭐⭐⭐⭐⭐"
        # You can now access the stars label and modify it
        stars_label.setText(line)  # For example, update the text (or star rating)

        # You can also print it to confirm
        #print(stars_label.text())  # Prints the current text of the stars label
